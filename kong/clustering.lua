...
local function get_shared_cert_expiration_time()
  ...
  cert = assert(openssl_x509.new(cert, "PEM"))
  local _, not_after, err = cert:get_lifetime()
  if err then
    ngx_log(ngx_ERR, "Error occurred during reading DP client certificate expiration time")
    return nil, err
  end
    ...
    while not exiting() do
      ...
      if err then
        ...
        ok, err = kong.db.clustering_data_planes:upsert({ id = node_id, }, {
          ...
          cert_exp_date = shared_cert_expiration_time
